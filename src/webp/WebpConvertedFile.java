...
class WebpConvertedFile {
  ...
  public void apply(@Nullable Object requestor) throws IOException {
    ...
    VirtualFile output = folder.createChildData(requestor, sourceFile.getNameWithoutExtension() + ".webp");
  ...
  public boolean convert(@NotNull WebpConversionSettings settings) {
    try {
      ...
      Logger.getInstance(WebpConvertedFile.class.getName()).error("Can't convert " + sourceFile.getPath(), e);
      return false;
    }
  }
  ...
  public static WebpConvertedFile create(@NotNull VirtualFile pngFile, @NotNull WebpConversionSettings settings) {
    try {
      ...
      if (image == null) {
        Logger.getInstance(WebpConvertedFile.class.getName()).warn("Can't read image: " + pngFile.getPath());
        return null;
      }

      String fileName = pngFile.getName();
      if (settings.skipNinePatches && endsWithIgnoreCase(fileName, ".9.png")) {
        return null;
      }

      // See if we find an alpha channel in this image and if so, return null
      if (settings.skipTransparentImages && (endsWithIgnoreCase(fileName, ".png") || endsWithIgnoreCase(fileName, ".gif"))) {
        if (ImageUtils.isNonOpaque(image)) {
          return null;
        }
      }
      ...
      Logger.getInstance(WebpConvertedFile.class.getName()).error("Can't convert " + pngFile.getPath(), e);
    }
    return null;
  }
